...
from pyqtgraph.opengl.items import GLTextItem, GLLinePlotItem  # <-- IMPORT THIS
...
# ... (all the other code in your file from TransformMPL down to VizScene) ...
# ... (I'm skipping it here to save space, but it should all be included) ...
...
class VizScene:
    ...
    # --- NEW FUNCTION ---
    def add_line(self, p1: NDArray, p2: NDArray, color=grey, width=1.0) -> GLLinePlotItem:
        """
        Adds a single 3D line segment to the visualization.

        :param NDArray p1: 3x1 start point
        :param NDArray p2: 3x1 end point
        :param tuple color: (r,g,b,a) color
        :param float width: line width
        :return: The GLLinePlotItem added to the scene
        """
        line_item = GLLinePlotItem.GLLinePlotItem(
            pos=np.array([p1, p2]),
            color=color,
            width=width,
            antialias=True
        )
        self.window.addItem(line_item)
        return line_item

    # --- NEW FUNCTION ---
    def remove_item(self, item):
        """
        Removes a specific graphics item from the window.
        """
        try:
            self.window.removeItem(item)
        except Exception as e:
            print(f"Warning: Could not remove item {item}. Error: {e}")
    ...
    def update(self, qs=None, As=None, poss=None):
        ...
        """
        if qs is not None:
            if isinstance(qs, (list, np.ndarray)) and isinstance(qs[0], (list, tuple, np.ndarray)):
    ...
    def hold(self, seconds: float|None=None):
        ...
        """
        if seconds is None:
            print("Visualization running. Close the window to continue.")
...
# ... (your existing class) ...
    ...
    def _get_joint_limits(self, arm):
        # Use the joint_limits from the arm object
        jt_lims = arm.qlim.T.copy() # N x 2
        
        # Convert revolute joints to degrees for slider
...
# ... (your existing class) ...
    def __init__(self, arm, link_colors=None, joint_colors=None, q0=None,
        ...
        arm_scale = np.max([np.max(np.abs(dh_array[0:,1:3])), 0.3])
        frame_scale = 1.5
        ee_scale = 2.0
...
# ... (your existing function) ...
        ...
        # This was 'i+1' before, which was correct inside the loop,
        # but after the loop, 'i' is n-1. We need the transform
        # to the tip, which is at index 'n'.
        A = self.arm.fk(q, self.n, base=True, tip=True)
        R = A[:3,:3]
        p = A[:3,3]
...
# ... (your existing class) ...
    def __init__(self, dh, jt='r',
        ...
        if np.abs(n_axis) < 1e-12 or le < 1e-12: # Added check for le
            R = np.eye(3)
        else:
            axis = axis / np.linalg.norm(axis)
            ang = np.arccos(np.clip(v1 @ v2 / (np.linalg.norm(v1) * np.linalg.norm(v2)), -1.0, 1.0)) # Added clip
